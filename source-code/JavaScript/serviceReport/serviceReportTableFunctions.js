document.addEventListener("DOMContentLoaded", () => {
    ...
    const generateQuotationButton = document.getElementById("generateServiceReport");
    if (generateQuotationButton) {
      generateQuotationButton.addEventListener("click", sendData);
    }
  
    document
      .querySelector("#serviceReportTable tbody")
  ...
  function addRow() {
    const tableBody = document.querySelector("#serviceReportTable tbody");
    const newRow = tableBody.insertRow();
    newRow.innerHTML = `
          <td><sl-input size="small" class="titleContent_InputField" type="text" name="item[]" placeholder="Unit"></sl-input></td>
          <td><sl-input size="small" class="titleContent_InputField" type="text" name="description[]" placeholder="Activity Performed"></sl-input></td>
  ...
  function updateGrandTotal() {
    const totalCells = document.querySelectorAll(
      "#serviceReportTable tbody .totalCell"
    );
  ...
  async function sendData() {
    ...
    const tableRows = document.querySelectorAll("#serviceReportTable tbody tr");
    const items = [];
    ...
    try {
      const response = await fetch(window.location.pathname, {
        ...
        body: JSON.stringify({ action: "serviceReportTABLE", items }),
      });
